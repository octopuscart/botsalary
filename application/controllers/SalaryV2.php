...
class SalaryV2 extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('Salary_modelV2');
    ...
    public function index() {
        ...
        if ($this->user_type == 'Admin') {
            $this->load->view('SalaryV2/dashboard', $data);
            ...
            $this->load->view('SalaryV2/salarylist', $data);
        }
        if ($this->user_type == 'SalaryManager') {
            redirect(site_url("SalaryV2/selectEmployee"));
        }
    }
    ...
    function create($employee_id) {
        ...
        if (isset($_POST["submit"])) {
            ...
            $salary_id = $this->Curd_model->insert('salary_v2', $salaryinsert);
            if (isset($salarydata["allowance_amount_no_mpf"])) {
                foreach ($salarydata["allowance_amount_no_mpf"] as $alkey => $alvalue) {
                    ...
                    if ($alvalue) {
                        ...
                        $this->Curd_model->insert('salary_allowances_apply_v2', $allowance_no_mpf);
                    }
                }
            }
            ...
            if ($salarydata["deduction"]) {
                ...
                $this->Curd_model->insert('salary_deduction_apply_v2', $deduation_no_mpf);
            }
            ...
            if ($salarydata["deduction_loan"]) {
                ...
                $this->Curd_model->insert('salary_deduction_apply_v2', $deduation_mpf);
            }
            redirect(site_url("SalaryV2/paySlip/$salary_id"));
        }
        $this->load->view('SalaryV2/addSalary', $data);
    }
    ...
    function selectEmployee() {
        ...
        $this->load->view('SalaryV2/employeelist', $data);
    }
    ...
    function paySlip($salary_id) {
        ...
        $salaryobj = $this->Salary_modelV2->employeeSalarySingle($salary_id);
        $data["salaryobj"] = $salaryobj;
        $data["allownce"] = $this->Salary_modelV2->employeeAllownceAll($salary_id);
        $data["paydate"] = date("M-Y", strtotime($salaryobj["salary_date"]));

        $data["deduction"] = $this->Salary_modelV2->employeeDuductionAll($salary_id);
        $data["employee"] = $this->Curd_model->get_single2('salary_employee', $salaryobj["employee_id"]);

        $this->load->view("SalaryV2/paySlip", $data);
    }
    ...
    function paySlipPdf($salary_id, $viewmode = "D") {
        ...
        $htmloutput = $this->load->view('SalaryV2/printSalaryBasePdf', $data, true);
...
//        $pdf->SetProtection(array('modify', 'copy', 'print'), $data["employee"]["employee_id"], "", 0, null);
    ...
    function salaryReportXls() {
        ...
        $salary_report = $this->Salary_modelV2->salaryData($a_date);
        $html = $this->load->view('SalaryV2/reportbase', array("salary_report" => $salary_report, "remark" => true, "showlogo" => false), true);
    ...
    function salaryReportPDF() {
        ...
        $salary_report = $this->Salary_modelV2->salaryData($a_date);
        $html = $this->load->view('SalaryV2/reportbase', array("salary_report" => $salary_report, "remark" => true), true);
        $filename = 'salary_report_' . $a_date . ".xls";
    ...
    function deletePayslip($id) {
        ...
        $this->db->where("id", $id);
        $this->db->delete("salary_v2");
        $salarydate = $this->input->get("salary_date");
        $this->db->where("salary_id", $salary_id);
        $query = $this->db->delete("salary_allowances_apply_v2");
        $this->db->where("salary_id", $salary_id);
        $query = $this->db->delete("salary_deduction_apply_v2");
        redirect(site_url("SalaryV2/selectEmployee?salary_date=$salarydate&select_month=1"));
    }
    ...
    function salaryReportV2() {
        ...
        $data["select_month"] = $a_date;
        $salarydata = $this->Salary_modelV2->salaryDatav2($a_date);
        $data["salary_report"] = $salarydata["salary_data"];
        $data["allownceslist"] = $salarydata["allownceslist"];
        $data["showimage"] = true;
        $this->load->view('SalaryV2/reportv2', $data);
    }
    ...
    function allowanceReports() {
        ...
        $data["showimage"] = true;
        $this->load->view('SalaryV2/allowanceReports', $data);
    }

    function allowanceReportsXls() {
        if ($this->user_type != 'Admin') {
            redirect('UserManager/not_granted');
        }
        $a_date = date("M-Y");
...
//        $pdf->SetProtection(array('modify', 'copy', 'print'), $data["employee"]["employee_id"], "", 0, null);
    ...
    function salaryReporV2tXls() {
        ...
        $html = $this->load->view('SalaryV2/reportbasev2', $data, true);
        $filename = 'salary_report_' . $a_date . ".xls";
        ob_clean();
        header("Content-Disposition: attachment; filename=$filename");
        header("Content-Type: application/vnd.ms-excel");
    ...
    function salaryReportV2PDF() {
        ...
        $data["salary_report"] = $salarydata["salary_data"];
        $data["allownceslist"] = $salarydata["allownceslist"];
        echo $html = $this->load->view('SalaryV2/reportbasev2', $data, true);
    }
